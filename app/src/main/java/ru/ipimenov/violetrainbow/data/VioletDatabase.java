...
@Database(entities = {Violet.class, ForeignViolet.class, MiniViolet.class, FavouriteViolet.class}, version = 2, exportSchema = false)
public abstract class VioletDatabase extends RoomDatabase {
    ...
    public static VioletDatabase getInstance(Context context) {
        synchronized (LOCK) {
            if (database == null) {
                database = Room.databaseBuilder(context, VioletDatabase.class, DB_NAME).fallbackToDestructiveMigration().build();
            }
        }
        return database;
    }
